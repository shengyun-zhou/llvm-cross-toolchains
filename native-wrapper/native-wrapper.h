...
#else
    TCHAR module_path[PATH_MAX + 1];
#if defined(__APPLE__)
    {
        uint32_t bufsize = sizeof(module_path);
        _NSGetExecutablePath(module_path, &bufsize);
    }
#else
    {
        ssize_t len = readlink("/proc/self/exe", module_path, PATH_MAX);
        module_path[len] = '\0';
    }
#endif
    TCHAR *sep2 = _tcsrchr(module_path, '/');
